...
Процедура ПриПолученииПараметров(ПараметрыКоманды) Экспорт
	...
	ПроверятьВJira = Булево(ПараметрыКоманды.Параметр("jira-status-check", Ложь));
...
Процедура ПроверитьКомментарийНаНаличиеУпоминанийЗадач(СтрокаВерсии, СледующаяВерсия)
	...
	Иначе
		РВ = Новый РегулярноеВыражение(СтрШаблон("%1-%2", ПрефиксЗадачи, "\d{1,8}"));
...
Функция ЗадачаВДопустимомСтатусе(НомерЗадачи)
	...
	Попытка
		...
		Аутентификация = Новый Структура("Пользователь, Пароль", "Jenkins", "=aBP=fTm)}28S3xD");
		Результат = КоннекторHTTP.Get(
			"https://jira.melonfashion.ru/rest/api/2/issue/"+НомерЗадачи+"?fields=status&fieldsByKeys=false",
    		Неопределено,
	...
	ИмяСтатуса = Результат.Получить("fields").Получить("status").Получить("name");
